"""
Google Sheets 데이터베이스 연결 및 CRUD 작업 (캐싱 최적화 버전)
"""
...
class GoogleSheetsDB:
    """Google Sheets 데이터베이스 클래스 (캐싱 최적화)"""
    
    def __init__(self):
        """Google Sheets 연결 초기화"""
        self.client = None
        self.spreadsheet = None
        self._worksheets_cache = {}  # 워크시트 캐시
    ...
    def _is_cache_valid(self, cache_key: str) -> bool:
        ...
        if cache_key not in self._last_cache_time:
            return False
        elapsed = (datetime.now() - self._last_cache_time[cache_key]).total_seconds()
    ...
    def clear_cache(self, sheet_name: Optional[str] = None):
        ...
        if sheet_name:
            # 특정 시트의 캐시만 클리어
            keys_to_remove = [k for k in self._data_cache.keys() if sheet_name in k]
    ...
    @retry_on_quota_error()
    def _fetch_all_records(self, sheet_name: str) -> List[Dict]:
        """시트의 모든 레코드 가져오기 (실제 API 호출)"""
        worksheet = self.get_worksheet(sheet_name)
        return worksheet.get_all_records()
    
    def _get_all_records_cached(self, sheet_name: str) -> List[Dict]:
        """캐시된 전체 레코드 가져오기"""
        cache_key = f"{sheet_name}_all_records"
        return self._get_cached_data(cache_key, self._fetch_all_records, sheet_name)
    ...
    def create_member(self, member_data: Dict) -> Dict:
        ...
        # 캐시 무효화
        self.clear_cache("Members")
        
        return member_data
    
    def get_member(self, member_id: str) -> Optional[Dict]:
        """회원 조회 (캐싱 적용)"""
        records = self._get_all_records_cached("Members")
    ...
    def create_schedule(self, schedule_data: Dict) -> Dict:
        ...
        # 캐시 무효화
        self.clear_cache("Schedules")
        
        return schedule_data
    
    def get_schedules_by_date(self, date: str) -> List[Dict]:
        """날짜별 스케줄 조회 (캐싱 적용)"""
        records = self._get_all_records_cached("Schedules")
    ...
    def create_contract(self, contract_data: Dict) -> Dict:
        ...
        # 캐시 무효화
        self.clear_cache("Contracts")
        
        return contract_data
    
    def get_contract_by_token(self, token: str) -> Optional[Dict]:
        """토큰으로 계약서 조회 (캐싱 적용)"""
        records = self._get_all_records_cached("Contracts")
    ...
    def get_recent_contracts(self, limit: int = 5) -> List[Dict]:
        """최근 계약서 조회 (캐싱 적용)"""
        records = self._get_all_records_cached("Contracts")
        # created_at 기준으로 정렬
    ...
    def create_signature(self, signature_data: Dict) -> Dict:
        ...
        # 캐시 무효화
        self.clear_cache("Signatures")
        
        return signature_data
    
    def get_signature(self, signature_id: str) -> Optional[Dict]:
        """서명 조회 (캐싱 적용)"""
        records = self._get_all_records_cached("Signatures")
    ...
    def get_statistics(_self) -> Dict:
        """통계 데이터 조회 (Streamlit 캐싱 적용)"""
        try:
            members = _self.get_all_members(status="active")
            schedules_today = _self.get_schedules_by_date(datetime.now().strftime("%Y-%m-%d"))
            pending_contracts = _self.get_pending_contracts()
            
            return {
                "total_members": len(_self.get_all_members()),
    ...
    def backup_to_csv(self, sheet_name: str, filepath: str):
        """시트를 CSV로 백업"""
        records = self._get_all_records_cached(sheet_name)
    ...
    def refresh_cache(self):
        ...
        self.clear_cache()
        # 주요 데이터 미리 로드
        self._get_all_records_cached("Members")
        self._get_all_records_cached("Schedules")
        self._get_all_records_cached("Contracts")
        self._get_all_records_cached("Signatures")